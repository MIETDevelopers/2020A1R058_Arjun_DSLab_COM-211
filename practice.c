...
/*void insertionByShop(struct Shop *newShop, int shopNumber)
{
    ...
    {
        headShop = newShop;
        newShop->walShopNumber = NULL;
    }
    else
    {
        while (shop != NULL && shop->shopNumber <= shopNumber)
        {
            index = shop;
...
int main()
{
	int operation, price, shopNumber;
	float quality;
	printf("\n\t\t\t\t\tNAMASTE\n\n");
	...
	do
    {
        printf("1.Register New Shop\n");
	printf("2.Registered Shops\n");
        printf("3.Shop with cheapest walnut\n");
	printf("4.Display all shops with Shop Number, Price and Quality\n");
	printf("5.Exit\n");
        scanf("%d", &operation);
	switch(operation)
        {
	...
	case 2:
		...
		break;
	case 3:
		printWPrice();		//calling printWPrice function
		break;
	case 4:
		printAll();		//calling printAll function
		break;
	case 5:
